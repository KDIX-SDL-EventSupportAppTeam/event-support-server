...
/**
 * 後出し割当。中央マスのうち state='EMPTY' のものを position 昇順で1つ選び、
 * 今チェックインしたブースを割り当てる。docs/.sdd/03-card-lifecycle/checkin.md
 *
 * トランザクション・行ロックが使えないため（D-7）、条件付き UPDATE の
 * affectedRows で直列化する。0 なら他リクエストが先に埋めたので取り直して再試行する。
 *
 * @returns 割り当てたマスの id。中央に EMPTY が無ければ null（＝実質すでに中央完成）。
 */
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    const [rows] = await db.query(
      `SELECT id, position FROM bingo_cells
       WHERE card_id = ? AND zone = 'CENTER' AND state = 'EMPTY'
...
/** カードの中央4マスが全て ACHIEVED になったかを判定する。 */
export async function isCenterComplete(db: DbClient, cardId: string): Promise<boolean> {
  const [rows] = await db.query(
    `SELECT COUNT(*) AS c FROM bingo_cells WHERE card_id = ? AND zone = 'CENTER' AND state <> 'ACHIEVED'`,
    [cardId],
  )
