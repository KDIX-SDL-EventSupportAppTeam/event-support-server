...
設計の解説は [docs/archive/legacy/designs/database.md](../archive/legacy/designs/database.md)、ビンゴ段階解放方式の設計は [docs/archive/2026-08-bingo-staged-unlock/](../archive/2026-08-bingo-staged-unlock/README.md) を参照。
...
`bingo_cards` / `bingo_cells` / `cell_assignment_logs` の3テーブルを追加。既存 `booths` に `is_active` / `duration_band` / `knowledge_level`、`check_ins` に `visit_order` / `cell_id`、`booth_ratings` に `prompt_context` / `scale` を追加（`rating` の `CHECK` は削除）。関連の環境変数: `RATING_SCALE`（既定3）・`CHECKIN_COOLDOWN_SEC`（既定0=無効）・`RECOMMENDER_TIMEOUT_MS`（既定1500）。詳細は [docs/archive/2026-08-bingo-staged-unlock/02-data-model/schema-changes.md](../archive/2026-08-bingo-staged-unlock/02-data-model/schema-changes.md)。
