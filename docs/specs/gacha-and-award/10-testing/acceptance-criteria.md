...
凡例: `[x]` = 自動テストまたは手元確認で満たした / `[ ]` = 未（実機リハーサル待ち等）
...
- [x] `ApiParticipantClient` からガチャのスタブ2本が**消えている**（`ParticipantClient` 型・Sample 実装からも削除）
- [x] 「使用する」の二重タップで API 呼び出しが1回（押下直後に `disabled`、`using` ガード）
- [x] 通信エラー後の再試行で**同じ冪等キー**が送られる（マウント時に1個生成し使い回す実装）
- [x] `409` で「他の端末で使い切った可能性」の文言が出て、枚数が再取得される
- [x] `403` で「準備中」表示になる
- [x] 残り0枚のとき使用ボタンが**表示されない**（disabled ではなく非表示）
- [x] 完了画面に「何枚目・何時何分・残り何枚」が出る
- [x] 完了画面からブラウザバックで使用確認画面に**戻れない**（`navigate(..., { replace: true })`）
- [x] スタッフに関する文言・画面・API が**どこにも無い**（`grep -ri "スタッフ" src/features/gachapon` が空）
- [x] ホームのボタンが残り枚数を表示し、0 枚で無効
- [x] 旧 Flask のエンドポイント文字列がリポジトリ内に1件も残っていない
- [ ] 実機（スマホ）で「素早く2回タップ」「機内モード→復帰で再試行」を確認（実機リハーサル）
