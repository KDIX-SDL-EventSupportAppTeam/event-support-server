...
4. **例外時の判定**: `idempotency_key` で SELECT する
   ...
   - 行が無い → `uk_gacha_coin` の衝突（同時実行の敗者）。**手順2から再試行する。**
     `COUNT(*)` は他者の INSERT で単調増加するため、高々「同時実行数」回で
     `success` か `NO_COINS_AVAILABLE`（`HAVING` 不成立）に必ず収束する。
     実装は保険の上限（`MAX_RETRY_ON_CONFLICT = 25`）まで回し、
     そこに達したら真に異常なので `500` を返す
     （さくらプロキシ経由なら HTTP が直列化されるため通常はループ1〜2周で抜ける。
     ローカル MySQL への真の並行では1回では収束しないことがある。
     [concurrency.md C-4](../10-testing/concurrency.md) / PR #84）
